...
| # | 항목 | 비고 |
|---|---|---|
| B1 | 규칙 확대 | 소득·자산 기준, 무주택 기간, 청약통장 납입횟수/예치금, 지역 거주요건, 재당첨 제한. `ApplicantProfile` 필드 추가 + 각 `EligibilityRule` 보강 |
| B2 | 일반공급 판정 | 가점제(무주택기간·부양가족·통장기간) 계산 + 추첨제. 트래픽 대부분이 여기 |
...
2. **B1 + B2** (규칙 확대 + 일반공급) — 이게 진짜 제품. 2~3주
