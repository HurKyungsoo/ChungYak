...
- **소득·자산 요건은 공유 컴포넌트로.** `IncomeRequirement`·`AssetRequirement` 를 5개 규칙이
  생성자 주입으로 쓴다(규칙마다 복붙 안 함). 유형별 기준값은 `application.yml`(`special-supply`,
  `income-reference`) — 제도가 바뀌면 배포 없이 수정.
...
- **거주요건·재당첨 제한**은 아직 판정에 넣지 않았다 (입력 필드는 있음). 화면 하단에 고지.
