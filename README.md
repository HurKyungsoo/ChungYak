...
- **경계값은 테스트로 못 박는다.** 혼인 84/85개월, 청약통장 6/24개월(규제지역),
  소득 유형별 상한(맞벌이 완화 포함), 공공주택 자산 상한(총자산 3.79억·자동차 3,708만).
  `EligibilityEngineTest` 가 이 경계를 지킨다 — 깨지면 규칙이 바뀐 것이다.
- **공통 요건은 공유 컴포넌트로.** 재당첨 제한·소득·자산을 `CommonRequirements` 로 묶어
  5개 규칙이 생성자 주입으로 쓴다(규칙마다 복붙 안 함). 각 요건은 PASS/FAIL/**MISSING**(판정 불가)
  을 사유와 함께 돌려주고, 규칙이 자기 판정에 접어 넣는다. 기준값은 `application.yml`
  (`special-supply`, `income-reference`, `re-win-restriction`, `account`, `region-residence`)
  — 제도가 바뀌면 배포 없이 수정.
...
- **소득·자산 요건**은 유형별 기준으로 판정에 반영(맞벌이 완화·공공주택 자산 포함).
  다만 소득·자산의 **세부 인정 기준**(자산 산정 방식, 부채 공제 등)까지는 안 본다.
- **재당첨 제한**은 판정에 반영(특공 평생 1회 + 지역별 제한 기간). 다만 부적격 당첨·계약
  미체결로 인한 제한은 안 본다.
- **거주요건**은 판정에 반영(규제지역 2년·수도권 1년 계속 거주). 미충족은 "우선공급 대상 아님,
  기타지역 물량은 가능"으로 안내. 지자체 조례에 따른 세부 기준까지는 안 본다.
